...
class _MediaCastScreenState extends State<MediaCastScreen> {
        ...
        actions: [
          if (!isSeen())
            TextButton(
              ...
              child: Text('SET SEEN', style: TextStyle(color: Theme.of(context).colorScheme.onSecondaryContainer)),
            ),
        ],
      ),
      body: Stack(
              ...
              child: Scrollbar(
                child: ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return MediaCastCard(castMember: widget.cast[index], rowClicked: (actor) => {castMemberClicked(actor)});
                  },
                ),
              ),
            ),
  ...
  Widget getAppBarTitle() {
    if (widget.movie != null) {
      return ListTile(
        title: AutoSizeText(widget.movie!.title!, minFontSize: 10, maxLines: 1),
        subtitle: Text('Released ${formatDateYearOnly(widget.movie!.releaseDate!)}'),
        contentPadding: EdgeInsets.zero,
      );
    } else {
      return ListTile(
        title: AutoSizeText(widget.tvShow!.title!, minFontSize: 10, maxLines: 1),
  ...
  Future<void> castMemberClicked(CastMember castMember) async {
    ...
    try {
      ...
      Navigator.push(context, MaterialPageRoute(builder: (context) => ActorDetails(actor: actor)));
    } catch (e) {
      showSnackbar('There was a problem loading the cast member', context);
    }
